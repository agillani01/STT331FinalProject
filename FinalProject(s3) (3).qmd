...
```{r}
# Create an interactive scatter plot of life expectancy vs happiness score for a specific year
data_specific_year <- data %>% filter(year == "X2022")  # change to the year you're interested in

p <- data_specific_year %>% 
  plot_ly(x = ~lex, y = ~happiness_score, type = 'scatter', mode = 'markers') %>%
  layout(title = "Interactive Plot: Life Expectancy vs Happiness Score",
